...
class QuizCreateConnection
{
    ...
    function UpdateQuiz($connection, $quizId, $title, $description, $timeLimit, $status, $totalMarks)
    {
        $sql = "UPDATE quizzes SET title = ?, description = ?, time_limit_minutes = ?, status = ?, total_marks = ? WHERE id = ?";
        $statement = $connection->prepare($sql);
        $statement->bind_param("ssisii", $title, $description, $timeLimit, $status, $totalMarks, $quizId);
    ...
    function GetQuestionByIdAndInstructor($connection, $questionId, $instructorId)
    {
        $sql = "SELECT q.* FROM questions q
                INNER JOIN quizzes qz ON q.quiz_id = qz.id
                WHERE q.id = ? AND qz.instructor_id = ?";
        $statement = $connection->prepare($sql);
        $statement->bind_param("ii", $questionId, $instructorId);
        $statement->execute();
        return $statement->get_result();
    }
